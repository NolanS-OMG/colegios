...
import {
  House, CalendarDays, BookOpenCheck, PencilLine, NotepadText, Headset, MoveLeft, LogOut,
...
const Dashboard = () => {
  ...
  return (
    <div className="flex relative">
      ...
      <div className="fixed bottom-0 right-0 bg-slate-50 p-3 shadow-sm rounded-full -translate-x-4 -translate-y-2">
        <MessageSquareText strokeWidth={2} color="#0f172a" size={22} />
      </div>
      ...
      <div className="custom-h-dashboard p-4 w-80 fixed left-0">
        <nav className="bg-slate-50 rounded-3xl shadow-md h-full">
          <ul className="flex flex-col h-full pb-4">
            ...
            <li className="px-6 my-2"><DashboardLink icon={<House strokeWidth={2} />} text="Home" path={`/usuario/alumno/${user.id}`} /></li>
            <li className="px-6 my-2"><DashboardLink icon={<CalendarDays strokeWidth={2} />} text="Calendario" path={`/usuario/alumno/${user.id}/calendario`} /></li>
            <li className="px-6 my-2"><DashboardLink icon={<BookOpenCheck strokeWidth={2} />} text="Calificaciones" path={`/usuario/alumno/${user.id}/calificaciones`} /></li>
            <li className="px-6 my-2"><DashboardLink icon={<PencilLine strokeWidth={2} />} text="Tareas" path={`/usuario/alumno/${user.id}/tareas`} /></li>
            <li className="px-6 my-2"><DashboardLink icon={<NotepadText strokeWidth={2} />} text="Material Didáctico" path={`/usuario/alumno/${user.id}/material`} /></li>
            <li className="px-6 my-2"><DashboardLink icon={<Headset strokeWidth={2} />} text="Asistencia" path={`/usuario/alumno/${user.id}/asistencia`} /></li>

            <li className="px-6 mt-auto mb-2"><Link to="/"><MoveLeft strokeWidth={1.5} size={20} className="inline" /><span className="text-sm inline ml-2">Volver al Inicio</span></Link></li>
      ...
      <div className="max-h-max w-full p-4 ml-80">
